...
# todo add title detector
# todo add best separator detector


class File:
    """
    1) Try to load data from self.path (if path provided)
    2) Try to download data from web (if no data loaded but URL provided)
    """
    def __init__(self, path=None, url=None, separator=',',
                 encoding="UTF-8", do_report=False, first_line_columns=False):
        """
        A File is used to load and store data as pd.DataFrames as .pkl files
        - download data from web as string (data is stored in .data as pd.DataFrame)
        - save data locally in a .pkl file
        - load data from .pkl file to self.data as pd.DataFrame
    ...
    def loading_sequence(self):     # core
        """
        1)
        ...
        if self.is_loaded():
            ...
            if self.have_path() or self.have_url():
                ...
                message = f'\n- Cannot load data from "{self.path}"'\
                          f'\n- Cannot download data from "{self.url}"'\
                          '\n\n Please check: \n- file path \n- URL \n- internet connection'
...
if __name__ == '__main__':
    ...
    def _test_1():
        ...
        path = 'data.pkl'
        f = File(url=url, path=path, do_report=True, separator=' ')
        f.save()
        f.load_file(path)
        print(f)
        f.del_file()
    ...
    def _test_3():
        ...
        path = 'data'
        f = File(url=url, path=path, first_line_columns=True, do_report=True)
        f.download_data()
        print(f)
        f.del_file()
